...
class GraphicsManager::GraphicsManagerImpl {
 ...
 private:
  ...
  struct Font {
    std::unordered_map<unsigned int, std::unordered_map<char, Character>>
        Characters;
  };
...
GraphicsManager::GraphicsManagerImpl::GraphicsManagerImpl(
  ...
  _window.reset(Helper::SDL_CreateWindow(
...
void GraphicsManager::GraphicsManagerImpl::DrawSprite(
  ...
  if (_imageRequestSize < _imageRequestQueue.size()) {
    _imageRequestQueue[_imageRequestSize++] = ImageDrawRequest{
        sprite.SpriteName,
        dst,
        center,
        sprite.RotationDegrees,
        sprite.Color,
        {sprite.DrawPriority.first, sprite.DrawPriority.second}};
  } else {
    _imageRequestQueue.emplace_back(ImageDrawRequest{
        sprite.SpriteName,
        dst,
        center,
        sprite.RotationDegrees,
...
void GraphicsManager::GraphicsManagerImpl::HandleRequests() {
  std::stable_sort(_imageRequestQueue.begin(),
  ...
  while (_textRequestQueue.empty() == false) {
    ...
    for (c = request.Text.begin(); c != request.Text.end(); c++) {
      ...
      _spriteRenderer->DrawSprite(ch.Texture, {xpos, ypos}, {w, h}, 0.0f,
                                  color);
...
void GraphicsManager::GraphicsManagerImpl::DrawText(
  ...
  _textRequestQueue.emplace(
      TextDrawRequest{text, {x, y}, font, fontSize, color});
}
...
GraphicsManager::GraphicsManagerImpl::~GraphicsManagerImpl() { SDL_Quit(); }
