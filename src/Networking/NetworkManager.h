...
class NetworkManager {
 public:
  ...
  void RemoveSession(uint32_t sessionId) {
    ...
    memcpy(leave_msg.Data.data(), &sessionId, sizeof(uint32_t));
 ...
 private:
  ...
  void AcceptSessionMessage(std::shared_ptr<Session> session) {
    boost::asio::async_read(
        session->Socket,
        boost::asio::buffer(&session->LastMessage, sizeof(NetworkMessage)),
        [this, session](std::error_code ec, std::size_t) {
          session->LastMessage.Size =
              boost::asio::detail::socket_ops::network_to_host_long(
                  session->LastMessage.Size);
          LOG_DEBUG(_logger, "Message Received; Session={}; Type={}; Size={}",
                    session->Id,
